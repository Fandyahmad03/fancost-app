...
export default function LandingPage() {
  ...
  // FUNGSI CEK FORMAT EMAIL (Biar errornya bahasa Indonesia)
  ...
  const handleAuth = async (e) => {
    ...
    // Cek dulu format emailnya sebelum dikirim ke Supabase
    if (!isValidEmail(email)) {
      setErrorMessage("Format email salah bos! Pake @ dan domain ya (cth: @gmail.com)");
      return;
    }

    if (!password) {
      setErrorMessage("Kata sandinya jangan dikosongin bos!");
      return;
    }
    ...
    if (isRegister) {
      ...
      if (error) setErrorMessage("Gagal Daftar: " + error.message);
      else setSuccessMessage("Berhasil daftar! Silakan cek email untuk verifikasi.");
    } else {
      const { error } = await supabase.auth.signInWithPassword({ email, password });
      if (error) setErrorMessage("Email atau kata sandi salah bos! Coba cek lagi.");
    }
    setLoading(false);
  };
  ...
  const handleResetPassword = async () => {
    ...
    if (!email) {
      setErrorMessage("Isi alamat emailnya dulu bos kalau mau reset kata sandi!");
      return;
    }

    // Cek format email untuk Lupa Password
    if (!isValidEmail(email)) {
      setErrorMessage("Format email salah bos! Pake @ dan domain ya (cth: @gmail.com)");
      return;
    }
    ...
    if (error) {
      ...
      setSuccessMessage("Link reset kata sandi sudah dikirim ke email kamu! 📩 Cek inbox/spam.");
    }
    setLoading(false);
  };
      ...
      <main className="relative z-20 max-w-6xl mx-auto w-full px-6 py-12 lg:py-16 grid grid-cols-1 lg:grid-cols-2 gap-12 flex-1 items-center">
        ...
        <div className="w-full max-w-md mx-auto">
          ...
          >
            ...
            {/* ALERT BOX UNTUK ERROR / SUKSES */}
            {errorMessage && (
              <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-5 p-3 rounded-xl bg-rose-500/10 border border-rose-500/30 text-rose-400 text-xs font-bold">
                ⚠️ {errorMessage}
              </motion.div>
            )}
            {successMessage && (
              <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-5 p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-xs font-bold">
                ✅ {successMessage}
              </motion.div>
            )}

            {/* TAMBAHKAN noValidate AGAR BROWSER TIDAK IKUT CAMPUR */}
            <form onSubmit={handleAuth} noValidate className="space-y-4 text-left">
              <div className="space-y-1.5">
                ...
                <input 
                  ...
                  className={`w-full px-4 py-3 bg-zinc-950 rounded-xl text-sm text-white outline-none transition-all placeholder:text-zinc-700
                    ${errorMessage ? "border border-rose-500/80 focus:border-rose-400 shadow-[0_0_15px_rgba(244,63,94,0.15)]" : "border border-zinc-800 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 shadow-inner"}`} 
                ...
                {!isRegister && (
                  <div className="flex justify-end mt-2">
                    <button 
                      ...
                      className="text-[10px] text-zinc-500 hover:text-cyan-400 font-bold transition-colors"
                    >
                      Lupa Kata Sandi?
                    </button>
                  </div>
              ...
              <motion.button 
                ...
                type="submit" 
                disabled={loading} 
                className="w-full bg-white hover:bg-zinc-200 text-black font-bold py-3.5 rounded-xl text-xs uppercase tracking-wider transition-all mt-2 disabled:opacity-40 flex items-center justify-center shadow-lg"
              >
                {loading ? (
